...
class MainBloc {
  ...
  final favoriteSperheroesSubject =
      BehaviorSubject<List<SuperheroInfo>>.seeded(SuperheroInfo.mocked);
  final searchedSperheroesSubject = BehaviorSubject<List<SuperheroInfo>>();
  ...
  void searchForSuperheroes(final String text) {
    ...
    searchSubscription = search(text).asStream().listen((searchResults) {
      if (searchResults.isEmpty) {
        ...
        searchedSperheroesSubject.add(searchResults);
  ...
  Stream<List<SuperheroInfo>> observeSearchedSuperheroes() =>
      searchedSperheroesSubject;

  Future<List<SuperheroInfo>> search(final String text) async {
    await Future.delayed(Duration(seconds: 1));
    List<SuperheroInfo> list = [];
    for (final superhero in SuperheroInfo.mocked) {
      if(superhero.name.toLowerCase().contains(text.toLowerCase())) {
        list.add(superhero);
      }
    }
    return list;
  ...
  void dispose() {
    stateSubject.close();
    favoriteSperheroesSubject.close();
    searchedSperheroesSubject.close();
